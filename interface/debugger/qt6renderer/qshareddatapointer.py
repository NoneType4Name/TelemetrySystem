...
class QSharedDataPointerSynth(AbstractSynth):
    ...
    def update(self) -> bool:
        ...
        self._values = [
            self._valobj.CreateValueFromData(
                QSharedDataPointerSynth.PROP_POINTER, d.data, d.type
            )
        ]
        ...
        if d.GetValueAsUnsigned():
            ...
            if value.IsValid():
                self._values.append(
                    self._valobj.CreateValueFromData(
                        QSharedDataPointerSynth.PROP_VALUE, value.data, value.type
                    )
                )
...
class QSharedDataPointer(SyntheticStruct, Generic):
    def __init__(self, pointer: SBValue, pointee_ctor: Callable[[SBValue]]):
